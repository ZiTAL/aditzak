...
for(const reading of ikusiShortReadings) for(const interpretation of reading.interpretations) {
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1').all(reading.form,'ikusi') as {id:string;payload:string}[];
  const row=rows.find(r=>{const a=JSON.parse(r.payload) as Analysis;return a.type==='nor-nork'&&a.nor===reading.nor&&
    a.nori===null&&a.nork===reading.nork&&a.mood===interpretation.mood&&a.tense===interpretation.tense;});
  const citation={sourceId:'euskaltzaindia-eab1979',locator:'142¹. or. (PDF 306), IKUSI: a gabeko adizkiak'};
  if(row){
    const analysis=JSON.parse(row.payload) as Analysis;Object.assign(analysis,{treatment:reading.treatment,allocutive:false,validation:'generated'});
    analysis.rawTags=[...new Set([...analysis.rawTags,'eab1979','note:a-less','NN1'])];analysis.citations.push(citation);
    updateOfficialNorNori.run(JSON.stringify(analysis),'euskaltzaindia-eab1979',row.id);
  }else{
    const analysis:Analysis={id:createHash('sha256').update(JSON.stringify(['eab1979-ikusi-short',reading.form,reading.nork])).digest('hex').slice(0,24),
      form:reading.form,lemma:'ikusi',kind:'synthetic',variety:'batua',mood:interpretation.mood,tense:interpretation.tense,
      type:'nor-nork',nor:reading.nor,nori:null,nork:reading.nork,treatment:reading.treatment,allocutive:false,
      affixes:[],rawTags:['eab1979','note:a-less','NN1'],baseForm:reading.form,origin:'rule',validation:'generated',
      citations:[citation],segmentation:null,history:[]};
...
for(const example of ikusiDativeExamples) {
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1').all(example.form,'ikusi') as {id:string;payload:string}[];
  ...
  const citation={sourceId:'euskaltzaindia-eab1979',locator:`143¹. or. (PDF 308), IKUSI datibozko adibidea: ${example.form}`};
  if(row){
    ...
    const analysis:Analysis={id:createHash('sha256').update(JSON.stringify(['eab1979-ikusi-dative',example])).digest('hex').slice(0,24),
      form:example.form,lemma:'ikusi',kind:'synthetic',variety:'batua',mood:'indicative',tense:'present',
...
// repaginated 1979 book prints ginderabilten. Preserve both source readings.
for(const interpretation of erabili1977Reading.interpretations) {
  const reading=erabili1977Reading;
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1')
    .all(reading.form,reading.lemma) as {id:string;payload:string}[];
  const row=rows.find(r=>{const a=JSON.parse(r.payload) as Analysis;return a.type==='nor-nork'&&a.nor===reading.nor&&
    a.nori===null&&a.nork===reading.nork&&a.mood===interpretation.mood&&a.tense===interpretation.tense;});
  const citation={sourceId:'euskaltzaindia-sintetikoa1977',locator:'818. or., ERABILI: ginderabiltzaten'};
  if(row){
    const analysis=JSON.parse(row.payload) as Analysis;
    Object.assign(analysis,{treatment:reading.treatment,allocutive:false,validation:'reviewed'});
    analysis.rawTags=[...new Set([...analysis.rawTags,'sintetikoa1977',reading.series])];analysis.citations.push(citation);
    updateOfficialNorNori.run(JSON.stringify(analysis),'euskaltzaindia-sintetikoa1977',row.id);
  }else{
    const analysis:Analysis={id:createHash('sha256').update(JSON.stringify(['sintetikoa1977-nor-nork',reading.form,
      reading.lemma,interpretation.mood,interpretation.tense,reading.nor,reading.nork])).digest('hex').slice(0,24),
      form:reading.form,lemma:reading.lemma,kind:'synthetic',variety:'batua',mood:interpretation.mood,tense:interpretation.tense,
      type:'nor-nork',nor:reading.nor,nori:null,nork:reading.nork,treatment:reading.treatment,allocutive:false,
      affixes:[],rawTags:['sintetikoa1977',reading.series],baseForm:reading.form,origin:'rule',validation:'reviewed',
      citations:[citation],segmentation:null,history:[]};
    lemmaInsert.run(reading.lemma,'synthetic');
    insert.run(analysis.id,analysis.form,reading.lemma,'batua',1,'euskaltzaindia-sintetikoa1977',JSON.stringify(analysis));
  }
}
// EGIN: retain the 1977 plural-NOR NN4 series and begitzate alongside the
// forms that replaced them in the 1979 publication.
for(const reading of egin1977Readings) for(const interpretation of reading.interpretations) {
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1')
    .all(reading.form,reading.lemma) as {id:string;payload:string}[];
  const row=rows.find(r=>{const a=JSON.parse(r.payload) as Analysis;return a.type==='nor-nork'&&a.nor===reading.nor&&
    a.nori===null&&a.nork===reading.nork&&a.mood===interpretation.mood&&a.tense===interpretation.tense;});
  const citation={sourceId:'euskaltzaindia-sintetikoa1977',locator:`829. or., EGIN ${reading.series}: ${reading.form}`};
  const validation:Analysis['validation']=reading.series==='NN4'?'generated':'reviewed';
  if(row){
    const analysis=JSON.parse(row.payload) as Analysis;
    Object.assign(analysis,{treatment:reading.treatment,allocutive:false,validation});
    ...
    const analysis:Analysis={id:createHash('sha256').update(JSON.stringify(['sintetikoa1977-egin',reading.form,
      interpretation.mood,interpretation.tense,reading.nor,reading.nork])).digest('hex').slice(0,24),form:reading.form,
      lemma:reading.lemma,kind:'synthetic',variety:'batua',mood:interpretation.mood,tense:interpretation.tense,
      type:'nor-nork',nor:reading.nor,nori:null,nork:reading.nork,treatment:reading.treatment,allocutive:false,
      affixes:[],rawTags:['sintetikoa1977',reading.series],baseForm:reading.form,origin:'rule',validation,
      citations:[citation],segmentation:null,history:[]};
    lemmaInsert.run(reading.lemma,'synthetic');
    insert.run(analysis.id,analysis.form,reading.lemma,'batua',1,'euskaltzaindia-sintetikoa1977',JSON.stringify(analysis));
  }
}
for(const reading of [...eginNnnPrinted,...eginNnnEllipsis,...eginNnnPlural]) for(const interpretation of reading.interpretations) {
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1').all(reading.form,'egin') as {id:string;payload:string}[];
  ...
  const locator=reading.evidence==='printed'?`${reading.printed}¹. or. (PDF ${reading.page}), EGIN ${reading.series}`:
    reading.evidence==='ellipsis'?'141¹. or. (PDF 304), EGIN NNN7 «eta abar»':
    `${reading.printed}¹. or. (PDF ${reading.page}), EGIN ${reading.series}, -gi- → -gizki-`;
  const citation={sourceId:'euskaltzaindia-eab1979',locator};
  const validation:Analysis['validation']=reading.evidence==='printed'&&reading.series!=='NNN4'?'reviewed':'generated';
  if(row){
    const analysis=JSON.parse(row.payload) as Analysis;
    Object.assign(analysis,{treatment:reading.treatment,allocutive:false,validation});
    analysis.rawTags=[...new Set([...analysis.rawTags,'eab1979',reading.series,`evidence:${reading.evidence}`])];
    analysis.citations.push(citation);updateOfficialNorNori.run(JSON.stringify(analysis),'euskaltzaindia-eab1979',row.id);
  }else{
    const analysis:Analysis={id:createHash('sha256').update(JSON.stringify(['eab1979-egin-nnn',reading.form,
      interpretation.mood,interpretation.tense,reading.nor,reading.nori,reading.nork])).digest('hex').slice(0,24),
      form:reading.form,lemma:'egin',kind:'synthetic',variety:'batua',mood:interpretation.mood,tense:interpretation.tense,
      type:'nor-nori-nork',nor:reading.nor,nori:reading.nori,nork:reading.nork,treatment:reading.treatment,allocutive:false,
      affixes:[],rawTags:['eab1979',reading.series,`evidence:${reading.evidence}`],baseForm:reading.form,origin:'rule',validation,
      citations:[citation],segmentation:null,history:[]};
    lemmaInsert.run('egin','synthetic');insert.run(analysis.id,analysis.form,'egin','batua',1,'euskaltzaindia-eab1979',JSON.stringify(analysis));
  }
}
for(const interpretation of egin1977NnnReading.interpretations) {
  const reading=egin1977NnnReading;
  const rows=db.prepare('SELECT id,payload FROM analyses WHERE form=? AND lemma=? AND base=1').all(reading.form,'egin') as {id:string;payload:string}[];
  const row=rows.find(r=>{const a=JSON.parse(r.payload) as Analysis;return a.type==='nor-nori-nork'&&a.nor===reading.nor&&
    a.nori===reading.nori&&a.nork===reading.nork&&a.mood===interpretation.mood&&a.tense===interpretation.tense;});
  const citation={sourceId:'euskaltzaindia-sintetikoa1977',locator:'830. or., EGIN NNN9: begiete'};
  if(row){
    const analysis=JSON.parse(row.payload) as Analysis;Object.assign(analysis,{treatment:'neutral',allocutive:false,validation:'reviewed'});
    analysis.rawTags=[...new Set([...analysis.rawTags,'sintetikoa1977','NNN9'])];analysis.citations.push(citation);
    updateOfficialNorNori.run(JSON.stringify(analysis),'euskaltzaindia-sintetikoa1977',row.id);
  }else{
    const analysis:Analysis={id:createHash('sha256').update('sintetikoa1977-egin-begiete').digest('hex').slice(0,24),
      form:reading.form,lemma:'egin',kind:'synthetic',variety:'batua',mood:interpretation.mood,tense:interpretation.tense,
      type:'nor-nori-nork',nor:reading.nor,nori:reading.nori,nork:reading.nork,treatment:'neutral',allocutive:false,
